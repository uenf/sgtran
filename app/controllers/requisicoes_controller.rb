class RequisicoesController < ApplicationController
  ...
  def fechar_viagem
    ...
    if params[:escolha_de_viagem].eql?("nova")
      ...
      if @viagem.valid?
        ...
        #Confirmacao.deliver.deliver_enviar_email_aceitar(corpo_do_email, destinatarios, @requisicao)
  ...
  def cancelar_requisicao_pelo_professor
    ...
    if @requisicao.cancelar_pelo_professor(params[:requisicao][:motivo_professor])
      ...
      #Confirmacao.deliver.deliver_email_cancelamento_professor(@requisicao)
    else
  ...
  def rejeitar_requisicao
    ...
    if @requisicao.rejeitar(motivo)
      #Confirmacao.deliver.deliver_enviar_email(corpo_email, destinatarios, @requisicao)
      redirect_to requisicao_path(@requisicao)
    else
  ...
  def cancelar_pelo_sistema
    ...
    if @requisicao.esta_aceita?
      ...
      if retorno
        #Confirmacao.deliver.deliver_enviar_email(corpo_do_email, destinatarios, @requisicao) if retorno
      end
    else
