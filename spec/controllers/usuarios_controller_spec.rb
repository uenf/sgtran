  ...
  describe "POST create" do
    ...
    describe "with valid params" do
      it "assigns a newly created usuario as @usuario" do
        Usuario.stub!(:new).with({'these' => 'params'}).and_return(
          mock_usuario(:save => true, :has_role! => nil))
        post :create, :usuario => {:these => 'params'}
        assigns[:usuario].should equal(mock_usuario)
      end

      it "redirects to the created usuario" do
        Usuario.stub!(:new).and_return(mock_usuario(:save => true, :has_role! => nil))
        post :create, :usuario => {}
        response.should redirect_to(usuario_url(mock_usuario))
      end

      it "should register user as admin" do
        usuario = mock_usuario(:save => true)
        Usuario.stub!(:new).and_return(usuario)
        usuario.should_receive(:has_role!)
        post :create, :usuario => {}
      end
    end
